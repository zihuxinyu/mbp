...
def saveTreeData(entity = None, operator = None) :
    ...
    listdata = []
    # 需要新建、修改的数据
    data = getargs2json("data")
    getTreelistData(data, listdata)

    #删除的数据处理
    data = getargs2json("removed")
    getTreelistData(data, listdata)
    #保存到数据库
    saveData(entity, listdata, operator)


def getTreelistData(data, listdata) :
    '''
    得到Tree控件列表中得所有信息
    ...
    for d in data :
        ...
        if "children" in d :
            # 遍历
            getTreelistData(d['children'], listdata)
