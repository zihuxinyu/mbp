    ...
    def create_model(self, table):
        ...
        class Temporal(self.DeclarativeBase):
            ...
            def _relation_repr(cls, rel):
                ...
                if rel.primaryjoin is not None and hasattr(rel.primaryjoin, 'right'):
                    right_lookup = lookup.get(rel.primaryjoin.right.table.name,
                ...
                if rel.secondary is not None:
                    ...
                    right_lookup = lookup.get(rel.secondaryjoin.right.table.name,
                                              '%s.c' % rel.secondaryjoin.right.table.name)
                    left_lookup = lookup.get(rel.secondaryjoin.left.table.name,
            ...
            def __repr__(cls):
                try:
                    ...
                    try:
                        ...
                        log.warn(
                            "A proper mapper could not be generated for the class %s, no relations will be created" % model_name)
                    s = ""
        ...
        for join_table in self.get_related_many_to_many_tables(table.name):
            ...
            if join_table not in self.tables:
                continue
            primary_column = \
                [c for c in join_table.columns if c.foreign_keys and list(c.foreign_keys)[0].column.table == table][0]
                                                                                             ...
                                                                                             relation(singular(
                                                                                                 name2label(
                                                                                                     ...
                                                                                                     related_table.schema)),
                                                                                                      ...
                                                                                                      primaryjoin=list(
                                                                                                          primary_column.foreign_keys)[
                                                                                                                      0].column == primary_column,
